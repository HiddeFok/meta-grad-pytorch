...
def plot_predictions(
    ...
    ax.plot(x_range, y_3, label="MetaGrad (Block)", color="tab:red", linewidth=2.5, linestyle="dotted")
...
if __name__ == "__main__":
    ...
    optimizer_metagrad_full = FullMetaGrad(model_metagrad_full.parameters(), sigma=3, D_inf=5)
    optimizer_metagrad_block = FullBlockMetagrad(model_metagrad_block.parameters(), sigma=3, D_inf=5)
    ...
    plot_and_save(
        losses_adagrad, losses_metagrad_full, losses_metagrad_block,losses_adam, fname_prefix="plot_meta"
    )
    plot_predictions(
        model_adagrad, model_metagrad_full, model_metagrad_block, model_adam, data_stream[0], data_stream[1]
    )
